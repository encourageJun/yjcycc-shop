...
public class GoodsZookeeperClient {
	...
	public boolean createTreeNode(UsingIpPort usingIpPort) {
		...
		try {
			Stat exist = zkTools.checkExists().forPath(GoodsRemoteConfig.BASE_PATH_SHOP_GOODS);
			if(exist == null){
				logger.info("------  第一次构建basepath:" + GoodsRemoteConfig.BASE_PATH_SHOP_GOODS);
				zkTools.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT)
				.withACL(ZooDefs.Ids.OPEN_ACL_UNSAFE).forPath(GoodsRemoteConfig.BASE_PATH_SHOP_GOODS);
			}
		
			String dataPath = GoodsRemoteConfig.BASE_PATH_SHOP_GOODS +"/"+ this.usingIpPort.getIndex();
			// 创建节点路径
