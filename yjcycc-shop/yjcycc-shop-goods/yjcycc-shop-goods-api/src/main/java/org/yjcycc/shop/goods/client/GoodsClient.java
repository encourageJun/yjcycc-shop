...
public class GoodsClient {
	...
	private static final Logger logger = LoggerFactory.getLogger(GoodsClient.class);
	/**
	 * 客户端用法举例
	 * @param args
	 */
...
//	}
	...
	public static Object getRemoteService (String serviceName){
		...
		try {
			//负载均衡 //简单轮询，无路由！
			UsingIpPort uip = GoodsZookeeperClient.getInstance().getOneLivingIpPort();
			if(uip == null){
				try{
					List<UsingIpPort>  list = GoodsZookeeperClient.readTreeStat();
					if(list != null && list.size() > 0){
						logger.info("本次RMI调用失败，下次调用会ok，请重试.");
						GoodsZookeeperClient.getInstance().updateIpPorts(list);
					}else{
