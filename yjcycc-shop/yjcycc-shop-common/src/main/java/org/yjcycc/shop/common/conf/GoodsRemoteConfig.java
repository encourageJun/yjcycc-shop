...
public class GoodsRemoteConfig {
	...
	private static final String RMI_CONFIG_FILE = "goods.rmi.properties";
	
	public static Charset charset = Charset.forName("utf-8");
	
	public static final String BASE_PATH_SHOP_GOODS = "/yycycc/shop/goods";// 节点路径
	...
	private GoodsRemoteConfig() {
		try{
			...
			serverIp0 = prop.getProperty("rmi.server.goods.ip.0" + dotenv);
			port0 = Integer.parseInt(prop.getProperty("rmi.server.goods.port.0" + dotenv) );
			serverIp1 = prop.getProperty("rmi.server.goods.ip.1" + dotenv);
			port1 = Integer.parseInt(prop.getProperty("rmi.server.goods.port.1" + dotenv) );
			serverIp2 = prop.getProperty("rmi.server.goods.ip.2" + dotenv);
			port2 = Integer.parseInt(prop.getProperty("rmi.server.goods.port.2" + dotenv) );
			serverIp3 = prop.getProperty("rmi.server.goods.ip.3" + dotenv);
			port3 = Integer.parseInt(prop.getProperty("rmi.server.goods.port.3" + dotenv) );
			...
			logger.info(">>>>>>>>>>>>>>>>> server ip0:"+serverIp0 + " server ip1:" +serverIp1 + " server ip2:"+serverIp2  + " server ip3:"+serverIp3);
			logger.info(">>>>>>>>>>>>>>>>> port0:"+port0 + " port1:"+port1 + "  port2:"+port2 + "  port3:"+port3);
			logger.info(">>>>>>>>>>>>>>>>> jmsBrokerUrl:"+jmsBrokerUrl);
			logger.info(">>>>>>>>>>>>>>>>> zookeeperConnUrl:"+zookeeperConnUrl);
			logger.info(">>>>>>>>>>>>>>>>> mongo:ip=" + getMongoServerIp() + ", port=" + getMongoServerPort() + ", username=" 	+ getMongoUsername() + ", password=" + getMongoPassword());
		}catch(Exception ex){
			ex.printStackTrace();
