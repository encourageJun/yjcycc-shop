...
public class RMIClient {
	...
	protected static AbstractZookeeperClient getClient(final String serviceName) {
		for (Entry<List<String>, String> entry : RMIConfig.getServicePathMap().entrySet()) {
			boolean f = entry.getKey().contains(serviceName);
			if (f) {
				final String basePath = entry.getValue();
				if (nodePathZkcMap.containsKey(basePath)) {
					return nodePathZkcMap.get(basePath);
				} else {
					...
					nodePathZkcMap.put(basePath, xzc);
					return xzc;
				}
			}
		}
	...
	protected static Object getProxyedService(final XxNode consumer,final XxNode producer , final Class<?extends Remote> serviceClass) throws RemoteException, NotBoundException {
		...
		Registry registry = RMIRegister.getRegistryClient(serverIp, port, RMIConfig.RMI_TIMEOUT_CONNECT);
